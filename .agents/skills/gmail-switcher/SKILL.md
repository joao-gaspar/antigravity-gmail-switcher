...
16. **O ANTIGRAVITY IDE ESTÁ SEMPRE ABERTO EM TODAS AS MÁQUINAS DO CONTEXTO**: É um fato estabelecido e permanente que o Antigravity IDE está sempre aberto em todos os computadores deste ambiente (EBBIM, LAPTOP e qualquer nova máquina adicionada). É PROIBIDO perguntar ao usuário se o IDE está aberto. O mecanismo de inicialização automática é o `.agents/hooks.json` que executa `gmail-switcher.ps1 check` via `PreInvocation` — isso inicia o `server.ps1`, sincroniza a telemetria e abre o painel com a URL correta automaticamente, sem nenhuma ação do usuário.
