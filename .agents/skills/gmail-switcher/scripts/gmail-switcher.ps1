...
# Self-healing candidate paths for accounts.json
$AccountsCandidates = @(
    (Join-Path $ScriptDir "..\accounts.json"),
    (Join-Path $ScriptDir "accounts.json"),
    "$env:USERPROFILE\.gemini\config\skills\gmail-switcher\accounts.json",
    "$env:USERPROFILE\.gemini\antigravity-ide\scratch\gmail-switcher\accounts.json",
    (Join-Path (Get-Location) "accounts.json"),
    (Join-Path (Get-Location) "..\accounts.json")
)

$AccountsFile = $null
foreach ($cand in $AccountsCandidates) {
    if (Test-Path $cand) {
        $AccountsFile = (Resolve-Path $cand).Path
        break
    }
}

if (-not $AccountsFile) {
    $targetDir = "$env:USERPROFILE\.gemini\config\skills\gmail-switcher"
    if (-not (Test-Path $targetDir)) { New-Item -ItemType Directory -Force -Path $targetDir | Out-Null }
    $AccountsFile = Join-Path $targetDir "accounts.json"
    if (-not (Test-Path $AccountsFile)) {
        @{ accounts = @() } | ConvertTo-Json -Depth 5 | Out-File $AccountsFile -Encoding UTF8
    }
}

$WatchFile = Join-Path (Split-Path $AccountsFile) "watch_state.json"

function Load-Accounts  { 
    if (Test-Path $AccountsFile) {
        try { return (Get-Content $AccountsFile -Raw -Encoding UTF8) | ConvertFrom-Json } catch {}
    }
    return [PSCustomObject]@{ accounts = @() }
}
function Save-Accounts  { 
    param($d)
    $parent = Split-Path $AccountsFile
    if (-not (Test-Path $parent)) { New-Item -ItemType Directory -Force -Path $parent | Out-Null }
    $d | ConvertTo-Json -Depth 10 | Out-File $AccountsFile -Encoding UTF8 
}
...
function Save-WatchState { 
    param($s)
    $parent = Split-Path $WatchFile
    if (-not (Test-Path $parent)) { New-Item -ItemType Directory -Force -Path $parent | Out-Null }
    $s | ConvertTo-Json -Depth 10 | Out-File $WatchFile -Encoding UTF8 
}
